...
@Component({
	...
	template: `
		...
		<form id="shopping-list-add" (ngSubmit)="onSubmit(f.value)" #f="ngForm">
			...
			<input type="text" id="item-name" required ngControl="name" [ngModel]="ingredient?.name" />
			<label for="item-amount">Amount</label>
			<input type="text" id="item-amount" required ngControl="amount" [ngModel]="ingredient?.amount" />
